...
class CacheManager {
    ...
    static func storeTempFile(_ remoteURL: URL) {
        URLSession.shared.downloadTask(with: remoteURL, completionHandler: { (location, _, error) in
            if error != nil {
                print("Error downloading temp file: \(error)")
            } else if let sourceURL = location, let destinationURL = CacheManager.tempFileURL(remoteURL) {
                do {
    ...
    static func storeApplicationSupportFile(_ remoteURL: URL, completionHandler: @escaping (_ localFileURL: URL?) -> Void) {
        ...
        URLSession.shared.downloadTask(with: request, completionHandler: { (location, _, error) in
            if error != nil {
                print("Error downloading Application Support file: \(error)")
                completionHandler(nil)
            } else if let sourceURL = location, let destinationURL = CacheManager.applicationSupportFileURL(remoteURL) {
                do {
                    try FileManager.default.moveItem(at: sourceURL, to: destinationURL)
